...
子代理**不能再次委派**（禁止嵌套）。每次调用默认最多 20 步，也可用 `maxSteps` 在
1–100 步内指定预算；若在工具调用后耗尽预算，内核会追加一次禁止调用工具的强制总结，
避免只返回过程性开场白。设置 `runInBackground=true` 可让主代理继续工作，并通过
`task_output` 查看状态，或通过 `task_stop` 停止继续调查并要求子代理基于已有证据总结。
超时与主会话压缩超时一致。
