...
describe("ChatSession context management", () => {
  ...
  it("lets the parent stop a background child and receive its evidence summary", async () => {
    const { ChatSession } = await import("../index.js");
    const dir = await mkdtemp(join(tmpdir(), "deepccc-child-stop-summary-"));
    streamTextMock.mockReturnValueOnce({ textStream: textStream("parent ready") });
    ...
    streamTextMock.mockImplementationOnce((options: { abortSignal: AbortSignal }) => ({
      fullStream: (async function* () {
        yield { type: "text-delta", text: "已找到一条证据。" };
        markChildStarted();
    ...
    generateTextMock.mockResolvedValueOnce({ text: "停止后的证据总结" });
    ...
    await childStarted;
    const stopped = await tools.task_stop.execute({ taskId: started.taskId });
    ...
    expect(generateTextMock).toHaveBeenCalledWith(expect.objectContaining({
      system: expect.stringContaining("最终总结器"),
      messages: expect.arrayContaining([
        expect.objectContaining({ content: expect.stringContaining("停止继续调查") }),
      ]),
    }));
  });
